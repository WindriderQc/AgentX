...
PromptTemplateSchema.index({ workspaceId: 1, category: 1 });
 ...
 * @param {ObjectId} workspaceId - Workspace ID (optional)
 * @param {Object} filters - Additional filters (category, search, etc.)
 * @returns {Array} - Array of templates
 */
PromptTemplateSchema.statics.getTemplatesForUser = async function(userId, workspaceId = null, filters = {}) {
  const query = {
    $or: [
  ...
  // Workspace filter
  if (workspaceId) {
    query.$or.push({ workspaceId: workspaceId });
  }
 ...
 */
PromptTemplateSchema.statics.getCategoryStats = async function(userId, workspaceId = null) {
  const query = {
    $or: [
      { isSystem: true },
      { userId: userId }
