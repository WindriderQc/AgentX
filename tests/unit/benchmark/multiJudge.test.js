...
describe('shouldUseMultiJudge', () => {
    it('returns true for categories that require multi-judge', () => {
        // Import what the categories config provides
        const { CATEGORY_MIN_JUDGE_TIER } = require('../../../config/categories');
        for (const [cat, tier] of Object.entries(CATEGORY_MIN_JUDGE_TIER || {})) {
            if (tier === 'standard' || tier === 'advanced' || tier === 'premium') {
                expect(typeof shouldUseMultiJudge(cat)).toBe('boolean');
            }
        }
    });
