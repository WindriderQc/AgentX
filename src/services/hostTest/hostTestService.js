 ...
 * Persists results to ModelRegistry.hostPerformance[].
 *
...
function getConfig() {
  return {
    timeoutMs: parseInt(process.env.HOST_TEST_TIMEOUT_MS, 10) || 60000,
    numPredict: parseInt(process.env.HOST_TEST_NUM_PREDICT, 10) || 64,
    contextFillPct: parseInt(process.env.HOST_TEST_CONTEXT_FILL_PCT, 10) || 25,
 ...
 */
async function warmUp(hostUrl, modelName, timeoutMs) {
  try {
    ...
    logger.warn('Host test warm-up failed', { hostUrl, modelName, error: err.message });
    throw new Error(`Warm-up failed: ${err.message}`);
  }
}
 ...
 * @returns {Promise<object>} HostPerformanceStepSchema-compatible snapshot
 */
async function testModelOnHost(modelName, hostUrl, options = {}) {
  ...
  const tokensPerSec = evalDurationSec > 0 ? Number((evalCount / evalDurationSec).toFixed(2)) : 0;
  const promptEvalTps = promptEvalDurationSec > 0
    ? Number((promptEvalCount / promptEvalDurationSec).toFixed(2)) : null;
  const timeToFirstTokenMs = promptEvalDuration > 0
    ? Number((promptEvalDuration / 1e6).toFixed(1)) : null;
  ...
  logger.info('Host test completed', {
    modelName, hostUrl, tokensPerSec, latencyMs: snapshot.latencyMs
  });
...
async function testAllModelsOnHost(hostUrl, options = {}) {
  const { hostId, onProgress } = options;
  ...
  for (let i = 0; i < models.length; i++) {
    ...
    if (onProgress) {
      try { onProgress(modelName, result, i, models.length); } catch (_) { /* ignore */ }
    }
  }
