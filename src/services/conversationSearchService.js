 ...
 * @param {ObjectId} [options.workspaceId] - Workspace ID for multi-tenancy
...
async function searchConversations(options) {
  ...
  try {
    ...
    // Stage 1: Match base filters (security + basic filters)
    ...
    // Stage 2: Full-text search (if query provided)
    if (query && query.trim()) {
      pipeline.push({
    ...
    const sortStage = buildSortStage(sortBy, !!query);
    ...
    if (query && query.trim()) {
      countPipeline.push({ $match: { $text: { $search: query } } });
    }
 ...
 * @param {ObjectId} [options.workspaceId] - Workspace ID
 * @param {Array<string>} options.tags - Tags to add
 * @returns {Promise<Object>} - Updated conversation
 */
async function addTagsToConversation(options) {
  const { conversationId, userId, workspaceId, tags } = options;
...
async function getUserTags(options) {
  const { userId, workspaceId, prefix, limit = 50 } = options;
