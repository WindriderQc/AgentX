...
const HOST_HARDWARE = {
    UGClawdX: { gpu: 'RTX 3090', vramGb: 24 },
    UGBrutal: { gpu: 'RTX 5070 Ti', vramGb: 16 },
    UGFrank: { gpu: 'RTX 3080 Ti', vramGb: 12 },
};
...
class UnifiedModels {
    ...
    filterModels() {
        ...
        this.filteredModels = this.allModels.filter(m => {
            ...
            // Provider
            const normalizedProvider = provider === 'n8n' ? 'n8n-webhook' : provider;
    ...
    updateStats() {
        ...
        if (totalEl) totalEl.innerText = gone.length ? `${active.length} + ${gone.length}` : String(active.length);
        ...
        const n8nCount = Array.isArray(s?.n8n?.webhooks) ? s.n8n.webhooks.length : active.filter(m => m?.provider === 'n8n-webhook').length;
        ...
        if (subEl) {
            const parts = [];
            if (n8nCount) parts.push(`${n8nCount} n8n`);
        ...
        if (totalSub) {
            const ollamaCount = active.filter(m => m.provider === 'ollama').length;
            const parts = [`${ollamaCount} Ollama`];
            const n8n = active.filter(m => m.provider === 'n8n-webhook').length;
            const custom = active.filter(m => m.provider === 'custom').length;
            if (n8n) parts.push(`${n8n} n8n`);
        ...
        const benchmarked = this.allModels.filter(m => m.benchmarkStats?.avgCompositeScore > 0);
    ...
    isModelLoaded(model) {
        ...
        return set.has(model.name) || set.has(model.name?.split(':')[0]) || false;
    }
    ...
    buildRowHTML(model) {
        const source = model.provider === 'n8n-webhook' ? 'n8n' : (model.provider || 'custom');
    ...
    getIconForSource(source) {
        ...
        if (source === 'n8n' || source === 'n8n-webhook') return '<i class="fas fa-cloud-bolt"></i>';
    ...
    showTotalPopout() {
        ...
        if (!active.length && !gone.length) {
            this.openStatPopout('Total Models', '<div class="popout-empty"><i class="fas fa-cubes"></i><div>No models found</div><div class="popout-muted">Models will appear here after sources sync.</div></div>');
            return;
        }
        ...
        // By host
        const hostCounts = this.getHostSummaries().map(host => [host.name, host.models.length]);
        ...
        `;
        this.openStatPopout(`Total Models: ${active.length}${gone.length ? ' + ' + gone.length + ' gone' : ''}`, html);
    }
    ...
    showStoragePopout() {
        const active = this.getActiveModels();
        if (!active.length) {
            this.openStatPopout('Storage', '<div class="popout-empty"><i class="fas fa-hdd"></i><div>No storage to report</div><div class="popout-muted">Add or sync models to populate storage stats.</div></div>');
            return;
        }
        ...
        const html = `
            <div class="popout-summary-grid">
                <div class="popout-summary-card">
                    ...
                    <span class="popout-summary-value">${this.formatBytes(totalBytes)}</span>
                </div>
                <div class="popout-summary-card">
                ...
                <div class="popout-summary-card">
                    ...
                    <span class="popout-summary-value">${sorted[0] ? this.formatBytes(sorted[0].size) : '--'}</span>
                </div>
            </div>
            <div class="popout-section">
                ...
                <div class="popout-bar-list">
                    ${Object.entries(hostStorage).sort((a,b) => b[1]-a[1]).map(([h, s]) => `
                        <div class="popout-bar-row">
                            ...
                            <span class="popout-bar-value">${this.formatBytes(s)}</span>
                        </div>
                    `).join('')}
                </div>
            </div>
                ...
                <div class="popout-bar-list">
                    ${sorted.map(m => `
                        <div class="popout-bar-row">
                            ...
                            <span class="popout-bar-value">${this.formatBytes(m.size)}</span>
                        </div>
                    `).join('')}
                </div>
            </div>
        `;
        this.openStatPopout(`Storage: ${this.formatBytes(totalBytes)}`, html);
    }

    /* ── Hosts Popout ──────────────────────────────────── */
    showHostsPopout() {
        const active = this.getActiveModels();
        const ollamaHosts = this.getHostSummaries();
            ...
            return `
                <div class="popout-host-card">
                    ...
                    <div class="popout-host-stats">
                        ...
                        <div class="popout-host-stat">
                            <span class="popout-host-stat-val">${this.formatBytes(host.totalSize)}</span>
        ...
        const n8nModels = active.filter(m => m.provider === 'n8n-webhook');
        const customModels = active.filter(m => m.provider === 'custom');
        const otherSection = (n8nModels.length || customModels.length) ? `
            <div class="popout-section">
                <h3 class="popout-heading"><i class="fas fa-plug-circle-bolt"></i> Other Providers</h3>
                ${n8nModels.length ? `
                    <div class="popout-sub-heading"><i class="fas fa-cloud-bolt"></i> n8n Webhooks (${n8nModels.length})</div>
                    <div class="popout-model-chips" style="margin-bottom:12px;">
                        ${n8nModels.map(m => `<span class="popout-model-chip">${escapeHtml(m.name)}</span>`).join('')}
                    </div>
                ` : ''}
                ${customModels.length ? `
    ...
    showBenchmarkPopout() {
        const benchmarked = this.allModels.filter(m => m.benchmarkStats?.avgCompositeScore > 0);
        const unbenchmarked = this.allModels.filter(m => !m.benchmarkStats?.avgCompositeScore && m.deployment?.status !== 'gone');

        if (!benchmarked.length) {
            this.openStatPopout('Benchmarks', '<div class="popout-empty"><i class="fas fa-flask-vial" style="font-size:32px; opacity:0.3;"></i><div>No models benchmarked yet</div><div class="popout-muted">Run benchmarks from the Benchmark page</div></div>');
            return;
        }
        ...
        `;
        this.openStatPopout(`Benchmarks — Avg ${avg.toFixed(1)}`, html);
    }
}
