...
(function () {
    ...
    // Inline tier selector — shows current tier as styled badge-like select,
    // with a ⚠ indicator when registry tier conflicts with size heuristic.
    function tierCell(judge) {
        const tier = judge.tier;
        const inferred = judge.inferredTier;
        const hasConflict = judge.hasConflict ||
            (judge.source !== 'discovered' && inferred && inferred !== tier);
        const c = tier
    ...
    function renderHostPanel(panel) {
        ...
        const bestRankBefore = judges[0] ? (TIER_RANK[judges[0].tier] || 0) : 0;
        ...
        <div class="roster-host-panel" data-host="${esc(panel.hostUrl)}"
            ...
            <!-- Tier coverage legend for this host -->
    ...
    function renderRoster(data) {
        ...
        // Bind tier select change — updates registry via PATCH, then re-renders
        container.querySelectorAll('.roster-tier-select').forEach(select => {
            select.addEventListener('change', async (e) => {
                ...
                const originalBg = sel.style.background;
                sel.disabled = true;
                sel.style.opacity = '0.5';
                try {
                    const resp = await fetch(
                        `/api/benchmark/judge-roster/${encodeURIComponent(modelName)}/tier`,
                        {
                            method: 'PATCH',
                            headers: { 'Content-Type': 'application/json' },
