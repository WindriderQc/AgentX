...
router.get('/', requireAuth, optionalWorkspaceContext, async (req, res) => {
  try {
    ...
    const templates = await PromptTemplate.getTemplatesForUser(userId, workspaceId, filters);
 ...
 */
router.get('/categories/stats', requireAuth, optionalWorkspaceContext, async (req, res) => {
  try {
    const userId = req.user.sub || req.user.id;
    const workspaceId = req.workspace ? req.workspace._id : null;

    const stats = await PromptTemplate.getCategoryStats(userId, workspaceId);
...
router.get('/:id', requireAuth, optionalWorkspaceContext, async (req, res) => {
  try {
    ...
    if (!template.isSystem && template.userId !== userId) {
      ...
      if (!req.workspace || !template.workspaceId ||
          template.workspaceId.toString() !== req.workspace._id.toString()) {
 ...
 */
router.post('/', requireAuth, optionalWorkspaceContext, async (req, res) => {
 ...
 * Body: { name, template, category, description, tags }
 */
router.put('/:id', requireAuth, optionalWorkspaceContext, async (req, res) => {
  const { name, template, category, description, tags } = req.body;
  ...
  try {
    ...
    if (existingTemplate.userId !== userId) {
      // Check workspace access
      if (!req.workspace || !existingTemplate.workspaceId ||
          existingTemplate.workspaceId.toString() !== req.workspace._id.toString()) {
        return res.status(403).json({ status: 'error', message: 'Access denied' });
      }
    }
 ...
 */
router.post('/:id/render', requireAuth, optionalWorkspaceContext, async (req, res) => {
...
router.post('/:id/duplicate', requireAuth, optionalWorkspaceContext, async (req, res) => {
  try {
    ...
    if (!sourceTemplate.isSystem && sourceTemplate.userId !== userId) {
      if (!req.workspace || !sourceTemplate.workspaceId ||
          sourceTemplate.workspaceId.toString() !== req.workspace._id.toString()) {
        return res.status(403).json({ status: 'error', message: 'Access denied' });
      }
    }
