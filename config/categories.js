...
const CATEGORY_JUDGE_REQUIREMENTS = {
  coding: {
    ...
    reason: 'Code correctness, debugging, and refactoring checks need solid reasoning depth.'
  },
  reasoning: {
  ...
  math: {
    ...
    reason: 'Math validation needs dependable step-by-step verification.'
  },
  knowledge: {
    label: BENCHMARK_CATEGORIES.knowledge.label,
    minTier: CATEGORY_MIN_JUDGE_TIER.knowledge,
    reason: 'Factual accuracy and explanation checks are governance-friendly for basic judges.'
  },
  instruction: {
    label: BENCHMARK_CATEGORIES.instruction.label,
    minTier: CATEGORY_MIN_JUDGE_TIER.instruction,
    reason: 'Instruction and format compliance are mostly explicit, so smaller judges are sufficient.'
  },
  creative: {
  ...
  translation: {
    ...
    reason: 'Translation comparison is pattern-heavy and can be governed with smaller judges.'
  }
};
