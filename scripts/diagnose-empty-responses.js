 ...
 * Queries BenchmarkResult for empty-response patterns to identify root cause.
 *
...
async function run() {
    ...
    // 1. Overall empty response stats by model
    ...
    // 2. Empty response details: done_reason, latency, eval_count
    ...
    const details = await BenchmarkResult.aggregate([
        { $match: { success: true, scoring_method: 'empty_response' } },
        {
            $group: {
                _id: {
                    model: '$model',
                    done_reason: '$truncation.done_reason'
                },
                count: { $sum: 1 },
                avg_latency: { $avg: '$latency' },
                min_latency: { $min: '$latency' },
                max_latency: { $max: '$latency' },
                avg_eval_count: { $avg: '$tokens' },
                hosts: { $addToSet: '$host' }
            }
        },
    ...
    for (const d of details) {
        ...
        console.log(`  count: ${d.count} | latency: ${d.min_latency}-${d.max_latency}ms (avg ${Math.round(d.avg_latency)}ms) | avg_tokens: ${Math.round(d.avg_eval_count)}`);
        console.log(`  hosts: ${d.hosts.map(h => h.replace(/https?:\/\//, '')).join(', ')}`);
        console.log();
    }

    // 3. Latency distribution for empty vs non-empty
    ...
    const latencyComp = await BenchmarkResult.aggregate([
        { $match: { success: true } },
        {
            $group: {
                ...
                count: { $sum: 1 },
                avg_latency: { $avg: '$latency' },
                p50_latency: { $avg: '$latency' }, // approximation
                min_latency: { $min: '$latency' },
                max_latency: { $max: '$latency' }
            }
        }
    ...
    // 4. Check execution settings for empty responses
    ...
    // 5. Check if models with 100% empty also have any non-empty results (across different batches)
